...
func TestMinimize(t *testing.T) {
	...
	tests := []struct {
		...
		// Test ArrayType step-halving chunk reduction on sys/test array_int.
		// A 50-element array should be reduced to non-consecutive target elements
		// (0x2, 0x5, 0x9, 0x15) via chunk reduction.
		{
			"test",
			"64",
			MinimizeCorpus,
			"test$array_int(&(0x7f0000000000)=[" +
				"0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0x0a, " +
			...
			0,
			func(p *Prog, callIndex int) bool {
				return regexp.MustCompile(`\b0x2\b.*\b0x5\b.*\b0x9\b.*\b0x15\b`).MatchString(string(p.Serialize()))
			},
			"test$array_int(&(0x7f0000000000)=[0x2, 0x5, 0x9, 0x15])\n",
			0,
		},
