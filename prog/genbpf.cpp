    ...
    for(cnt = 0; cnt < NINSNS; ){
        switch(rand() % 24){
            ...
            case JMPOP:
                ...
                break;            
            case HELPER:
                // genHelper(regStates, bpfBytecode, &cnt);
                break;
            case SUBFUNC:
                // genSubFunc(regStates, bpfBytecode, &cnt);
                break;
        }
    }
...
void genALUOP(struct regState *regStates, struct bpf_insn *bpfBytecode, int *cnt) {
    ...
    */
    ...
    updateByteCode(genNewInsn, bpfBytecode, cnt, insn);
}
...
void genLSOP(struct regState *regStates, struct bpf_insn *bpfBytecode, int *cnt, int maxMaps) {
    ...
    */
    ...
    short int off = 0;
    int32_t imm32, imm64, fdIdx;
    struct bpf_insn insn;
    bool genNewInsn = false;

    switch(rand() % 6) {
        case 0:
            if (!initRegPtr(src, Bit64, regStates, bpfBytecode, cnt)) break;
        ...
        case 1:
            ...
            if (!initRegPtr(dst, Bit64, regStates, bpfBytecode, cnt)) break;
        ...
        case 3:
            ...
            // if (!initRegScalar(src, Bit64, regStates, bpfBytecode, cnt)) break;
            if (!initRegPtr(dst, Bit64, regStates, bpfBytecode, cnt)) break;
            if (BPF_ATOMIC_OP_Constrait(size, op, dst, src, off)) {
        ...
        case 4: {
            ...
            genNewInsn = true;
            updateByteCode(genNewInsn, bpfBytecode, cnt, insns1[0]);
            insn = insns1[1];
            ...
            genNewInsn = true;
            updateByteCode(genNewInsn, bpfBytecode, cnt, insns2[0]);
            insn = insns2[1];
