...
func (repo *SeriesRepository) ListLatest(ctx context.Context, filter SeriesFilter,
	...
	if filter.Name != "" {
		conds = append(conds, `SEARCH_SUBSTRING(Series.TitleSubstringTokens, @name, relative_search_type => 'phrase')
OR EXISTS(SELECT 1 FROM Patches WHERE Patches.SeriesID = Series.ID
   AND SEARCH_SUBSTRING(Patches.TitleSubstringTokens, @name, relative_search_type => 'phrase')
)`)
