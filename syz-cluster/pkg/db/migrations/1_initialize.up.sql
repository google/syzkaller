...
-- SessionSteps tracks the current workflow steps state.
CREATE TABLE SessionSteps (
    SessionID STRING(36) NOT NULL,
    StepName STRING(128) NOT NULL,
    StartedAt TIMESTAMP,
    FinishedAt TIMESTAMP,
    State STRING(32) NOT NULL,
    CONSTRAINT StateEnum CHECK (State IN ('finished', 'failed', 'in_progress')),
    CONSTRAINT FK_Session FOREIGN KEY (SessionID) REFERENCES Sessions (ID),
) PRIMARY KEY(SessionID, StepName);
