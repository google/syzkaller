...
func run(configFile string, exitOnUpgrade, autoUpdate bool, syzkallerDir, name string) error {
	...
	if cfg.MCP {
		var firstTarget *TargetConfig
		for _, tcfg := range cfg.Targets {
			firstTarget = tcfg
			break
		}
		http.Handle("/", mcpHandler(initState(firstTarget, syzkallerDir, name), workdir, cache))
		select {}
	}
...
func (s *Server) executeJob(ctx context.Context, req *dashapi.AIJobPollResp) (out map[string]any, err error) {
	...
	if tcfg == nil {
		return nil, fmt.Errorf("dashboard requested TargetOS %q TargetArch %q, but agent does not support it",
			agentOS, agentArch)
	}
