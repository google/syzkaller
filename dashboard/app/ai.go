...
func bugJobCreate(ctx context.Context, workflow string, typ aidb.WorkflowType, bug *Bug) error {
...
func autoCreateAIJob(ctx context.Context, bug *Bug, bugKey *db.Key) (bool, error) {
	...
	for _, job := range jobs {
		...
		if !job.Finished.Valid ||
			...
			delete(workflows, aidb.WorkflowType(job.Workflow))
		}
	}
...
func workflowsForBug(bug *Bug, manual bool) map[aidb.WorkflowType]bool {
	workflows := make(map[aidb.WorkflowType]bool)
	if strings.HasPrefix(bug.Title, "KCSAN: data-race") {
		workflows[aidb.WorkflowAssessmentKCSAN] = true
	}
	if manual {
		// Types we don't create automatically yet, but can be created manually.
		if bug.ReproLevel > dashapi.ReproLevelNone {
			workflows[aidb.WorkflowPatching] = true
		}
	}
	return workflows
}
