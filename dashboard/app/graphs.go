...
type uiBugsGraphPage struct {
	Header            *uiHeader
	Months            *uiSlider
	GraphKernelHealth *uiGraph
	GraphFixDuration  *uiGraph
	BugLifetimes      []uiBugLifetime
}
...
// nolint: dupl
func handleGraphBugs(c context.Context, w http.ResponseWriter, r *http.Request) error {
	...
	data := &uiBugsGraphPage{
		Header:            hdr,
		GraphKernelHealth: createBugsGraph(c, bugs),
		GraphFixDuration:  createFixDurationGraph(),
		BugLifetimes:      createBugLifetimes(c, bugs, causeBisects),
	}
...
func createBugsGraph(c context.Context, bugs []*Bug) *uiGraph {
	...
	return &uiGraph{
		Headers: []string{"TotalOpen", "TotalReported", "TotalFixed"},
		Columns: columns,
	}
}

func createFixDurationGraph() *uiGraph {
	return &uiGraph{
		Headers: []string{"TotalOpen"},
	}
}
...
func createBugLifetimes(c context.Context, bugs []*Bug, causeBisects map[string]*Job) []uiBugLifetime {
	...
	for i, bug := range bugs {
		ui := uiBugLifetime{
			Reported: bug.FirstTime, //!!! not exactly reported
		}
		fixed := bug.FixTime
...
func createCheckBox(r *http.Request, caption string, values []string) *uiCheckbox {
	id := caption //!!! turn into proper id
	ui := &uiCheckbox{
		ID:      id,
		Caption: caption,
		vals:    r.Form[id],
	}
	//!!! filter selMetrics against allMetrics
	if len(ui.vals) == 0 {
	...
	for _, val := range values {
		ui.Values = append(ui.Values, &uiCheckboxValue{
			ID: val,
			//!!! use this as caption and form ID
