...
func makeGvisor(target *targets.Target, image, srcDir string, modules []*vminfo.KernelModule) (*Impl, error) {
	if len(modules) != 0 {
		return nil, fmt.Errorf("gvisor coverage does not support modules")
	}
	if image == "" {
		return nil, fmt.Errorf("gvisor coverage requires the image")
	}
	// Coverage data is symbolized by runsc, which execs to a sidecar binary,
	// so we need to extract the image here.
	dir, err := os.MkdirTemp("", "syz-gvisor-image")
	if err != nil {
		return nil, err
	}
	defer os.RemoveAll(dir)
	runsc, err := gvisor.Extract(image, dir)
	if err != nil {
		return nil, err
	}
	frames, err := gvisorSymbolize(runsc, srcDir)
	if err != nil {
		return nil, err
	}
...
func gvisorParseLine(s *bufio.Scanner) (*Frame, error) {
	...
	for i := range ints {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to parse number %q: %w", match[i+2], err)
		}
		ints[i] = int(x)
	}
	frame := &Frame{
...
// Source files are reported relative to the gVisor module root, optionally prefixed with
// the module path (`gvisor.dev/gvisor/`) or with a bazel output directory (for
// autogenerated files).
var gvisorLineRe = regexp.MustCompile(
	`^(?:gvisor\.dev/gvisor/|bazel-out/[^/]+/bin/)?([^:]+):([0-9]+)\.([0-9]+),([0-9]+)\.([0-9]+)$`)
