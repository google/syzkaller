...
func buildTestBinary(t *testing.T, target *targets.Target, test *Test, dir string) string {
	...
	if err := osutil.WriteFile(src, []byte(code)); err != nil {
		t.Fatal(err)
	}
...
func TestResolveLineToPCs(t *testing.T) {
...
`
	...
	modules := []*vminfo.KernelModule{{Path: bin}}
	impl, err := backend.Make(cfg, modules)
	assert.NoError(t, err)

	var computeSym, mainSym *backend.Symbol
	for _, s := range impl.Symbols {
		switch s.Name {
		case "compute":
			computeSym = s
		case "main":
	...
	assert.NotNil(t, computeSym)
	assert.NotNil(t, mainSym)
	assert.Len(t, computeSym.PCs, 5)
	assert.Len(t, mainSym.PCs, 1)
	...
	tests := []struct {
	...
	}{
		{
			...
			file:    "main.c",
			line:    3,
			wantPCs: []uint64{computeSym.PCs[0]},
		},
		{
			name:    "then-branch-statement",
			file:    "main.c",
			line:    4,
			wantPCs: []uint64{computeSym.PCs[3]},
		},
		{
			name:    "then-branch-return",
			file:    "main.c",
			line:    5,
			wantPCs: []uint64{computeSym.PCs[4]},
		},
		{
			name:    "else-branch-statement",
			file:    "main.c",
			line:    7,
			wantPCs: []uint64{computeSym.PCs[1]},
		},
		{
			name:    "else-branch-return",
			file:    "main.c",
			line:    8,
			wantPCs: []uint64{computeSym.PCs[2]},
		},
		{
			name:    "main-body",
			file:    "main.c",
			line:    12,
			wantPCs: []uint64{mainSym.PCs[0]},
		},
		{
			name:    "invalid-line-zero",
			file:    "main.c",
			line:    0,
			wantErr: "both filePath and a positive line number must be provided",
		},
		{
			name:    "line-before-first-function",
			file:    "main.c",
			line:    1,
			wantErr: "no KCOV coverage PC found for line 1",
		},
		{
			name:    "unknown-file",
			file:    "unknown.c",
			line:    3,
			wantErr: `no KCOV coverage points found for file "unknown.c"`,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			pcs, err := impl.ResolveLineToPCs(tt.file, tt.line)
			if tt.wantErr != "" {
				assert.Error(t, err)
				assert.Contains(t, err.Error(), tt.wantErr)
			} else {
				assert.NoError(t, err)
				assert.Equal(t, tt.wantPCs, pcs)
			}
		})
	}
}
