...
type readSyzSpecArgs struct {
	...
	LineCount int    `jsonschema:"Number of lines to return, capped at 100." json:",omitempty"`
}
...
func resolveTargetFiles(syzFS *syzspec.SyzFS, file string) ([]string, error) {
	if file == "" {
		// When file is empty, syz-grepper searches across all description files.
		// These are target-specific description files excluding test/ subdirectory.
		return syzFS.DescriptionFiles(), nil
	}

	fileInfo, err := fs.Stat(syzFS, file)
	if err == nil && fileInfo.IsDir() {
		var targetFiles []string
		err = fs.WalkDir(syzFS, file, func(p string, d fs.DirEntry, err error) error {
			if err != nil {
				return err
			}
			if !d.IsDir() {
...
func grepSyzSpec(syzFS *syzspec.SyzFS, expression, file string) (string, error) {
	...
	targetFiles, err := resolveTargetFiles(syzFS, file)
	if err != nil {
		return "", err
	}
...
func limitOutputBytes(res string) string {
	if len(res) > maxOutputBytes {
		res = res[:maxOutputBytes]
		for res != "" && !utf8.ValidString(res) {
			res = res[:len(res)-1]
		}
