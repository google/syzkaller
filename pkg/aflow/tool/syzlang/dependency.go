...
func getASTDescriptionInfo(syzkallerDir, osTarget string) (*astDescriptionInfo, error) {
	...
	sysFS := syzspec.NewSyzFS(syzkallerDir, osTarget)
	files := sysFS.DescriptionFiles()
...
func ResolveSyzlangDependencies(ctx *aflow.Context, _ codeFixerState, args CodeFixerArgs) (map[string]any, error) {
	if args.SyzProgram == "" {
