...
type prepareResult struct {
	//!!! do we really need these json names?
	Index index `json:"codesearch-index"`
}

type fileIndexArgs struct {
	//!!! do we really need these json names?
	SourceFile string `json:"source-file" jsonschema:"Source file path."`
}
...
type defCommentArgs struct {
	SourceFile string `json:"source-file" jsonschema:"Source file path that references the entity. It helps to restrict scope of the search, if there are different definitions with the same name in different source files."`
...
type defSourceArgs struct {
	SourceFile   string `json:"source-file" jsonschema:"Source file path that references the entity. It helps to restrict scope of the search, if there are different definitions with the same name in different source files."`
	Name         string `json:"name" jsonschema:"Name of the entity of interest."`
	IncludeLines bool   `json:"include-lines" jsonschema:"Whether to include line numbers in the output or not. Line numbers may distract you, so ask for them only if you need to match lines elsewhere with the source code."`
}

// nolint: lll
type defSourceResult struct {
	Missing    bool   `json:"missing,omitempty" jsonschema:"Set to true if the entity with the given name does not exist."`
	SourceFile string `json:"source-file,omitempty" jsonschema:"Source file path where the entity is defined."`
	SourceCode string `json:"source-code,omitempty" jsonschema:"Source code of the entity definition. It is prefixed with line numbers, so that they can be referenced in other tool invocations."`
}
