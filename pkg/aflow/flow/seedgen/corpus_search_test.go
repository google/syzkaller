...
func TestCorpusCodeSearch(t *testing.T) {
	...
	_, cachedID, err := aflow.CacheObject(ctx, "corpus-execution", "test-desc", func() (corpusData, error) {
		return cData, nil
	})
	...
	tests := []struct {
		...
		},
		{
			...
			wantContains: []string{
				"No corpus programs found that reaching function \"non_existent_function\"",
			},
		},
	}
