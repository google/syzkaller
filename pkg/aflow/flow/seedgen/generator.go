...
2. CODE-FIXER OSCILLATION & STAGNATION:
   - Calling 'code-fixer' 3 or more times with identical or near-identical syzlang code.
...
3. STRATEGY LOCK-IN & ZERO PROGRESS:
   - Spending more than 15 turns pursuing a single unreachable syscall path or unhelpful setup
     sequence without exploring alternative call sequences.
...
- Set Stop = false IF the subagent is introducing distinct new syscalls, testing fresh setup sequences, or actively
  progressing.`,
	},
...
` + syzlang.VMAAndMemoryPointerConstraints + `
...
{{end}}{{else if .Func}}
Note: The exact PC(s) are located inside the inlined function '{{.Func}}'
which is called within the target function.
{{end}}

{{if .EnvironmentPrompt}}Target Environment:
{{.EnvironmentPrompt}}
