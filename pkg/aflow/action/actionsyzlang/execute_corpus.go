...
func executeCorpusAction(ctx *aflow.Context, args ExecuteCorpusArgs) (ExecuteCorpusResult, error) {
	...
	desc := fmt.Sprintf("corpus-execution-v2-%v-%v", args.KernelCommit, corpusSig)
	_, cachedID, err := aflow.CacheObject(ctx, "corpus-execution", desc, func() (syzspec.CorpusData, error) {
		target, err := prog.GetTarget(args.TargetOS, args.TargetArch)
		if err != nil {
			return syzspec.CorpusData{}, fmt.Errorf("unknown target: %s/%s", args.TargetOS, args.TargetArch)
		}
		sysTarget := targets.Get(args.TargetOS, args.TargetArch)
		if sysTarget == nil {
			return syzspec.CorpusData{}, fmt.Errorf("unknown sys target: %s/%s", args.TargetOS, args.TargetArch)
		}

		log.Logf(0, "aflow: executing corpus from %q", args.CorpusPath)
		progs, err := db.ReadCorpus(args.CorpusPath, target)
		if err != nil {
			return syzspec.CorpusData{}, fmt.Errorf("failed to read corpus: %w", err)
		}
		if len(progs) == 0 {
			return syzspec.CorpusData{}, fmt.Errorf("corpus is empty")
		}
		...
		if err != nil {
			return syzspec.CorpusData{}, fmt.Errorf("failed to create workdir: %w", err)
		}

		cfg, err := crash.BuildConfig(targetConfig, workdir)
		if err != nil {
			return syzspec.CorpusData{}, fmt.Errorf("failed to build config: %w", err)
		}
		...
		if err != nil {
			return syzspec.CorpusData{}, fmt.Errorf("failed to submit corpus for execution: %w", err)
		}

		programsMap, progHashes, pcToProgHashes := collectPCs(results, progs, sysTarget, args.Type)

		finalFunctionMap, err := symbolizeFunctionMap(pcToProgHashes, sysTarget, args.KernelObj)
		if err != nil {
			return syzspec.CorpusData{}, err
		}

		finalSyscallMap := buildSyscallMap(progs, programsMap, progHashes)

		return syzspec.CorpusData{
			Programs:    programsMap,
			FunctionMap: finalFunctionMap,
			SyscallMap:  finalSyscallMap,
		}, nil
	})
	if err != nil {
...
func collectPCs(results []*queue.Result, progs []*prog.Prog, sysTarget *targets.Target, vmType string) (
	map[string]string, []string, map[uint64]map[string]bool,
) {
	programsMap := make(map[string]string)
	progHashes := make([]string, len(progs))
	pcToProgHashes := make(map[uint64]map[string]bool)

	for i, res := range results {
		if res == nil || res.Err != nil || res.Info == nil {
			continue
		}
	...
	return programsMap, progHashes, pcToProgHashes
}

func symbolizeFunctionMap(pcToProgHashes map[uint64]map[string]bool,
	sysTarget *targets.Target, kernelObj string) (map[string][]string, error) {
	functionMap := make(map[string]map[string]bool)
	if len(pcToProgHashes) > 0 {
		uniquePCs := slices.Collect(maps.Keys(pcToProgHashes))
		vmlinux := filepath.Join(kernelObj, sysTarget.KernelObject)
		...
		if err != nil {
			return nil, fmt.Errorf("failed to batch symbolize coverage: %w", err)
		}
	...
	for fn, hashesMap := range functionMap {
		finalFunctionMap[fn] = slices.Collect(maps.Keys(hashesMap))
	}
	return finalFunctionMap, nil
}

func buildSyscallMap(progs []*prog.Prog, programsMap map[string]string, progHashes []string) map[string][]string {
	syscallMap := make(map[string]map[string]bool)
	for i, p := range progs {
		pHash := progHashes[i]
		if _, ok := programsMap[pHash]; !ok {
			continue
		}
		for _, call := range p.Calls {
			name := call.Meta.Name
			if syscallMap[name] == nil {
				syscallMap[name] = make(map[string]bool)
			}
			syscallMap[name][pHash] = true
		}
	}

	finalSyscallMap := make(map[string][]string)
	for name, hashesMap := range syscallMap {
		finalSyscallMap[name] = slices.Collect(maps.Keys(hashesMap))
	}
	return finalSyscallMap
}
