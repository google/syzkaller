...
func (vctx *verifyContext) errorf(who, msg string, args ...any) {
	noteError(&vctx.err, fmt.Sprintf("action %v: %v", who, msg), args...)
}
...
func requireJsonSchema[T any](vctx *verifyContext, who, what string) {
	for _, field := range reflect.VisibleFields(reflect.TypeFor[T]()) {
		if field.Tag.Get("jsonschema") == "" {
			vctx.errorf(who, "%v field %v must have a jsonschema tag with description",
				what, field.Name)
		}
	}
}
