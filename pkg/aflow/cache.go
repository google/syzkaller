...
type cacheEntry struct {
	...
	usageCount int
	lastUsed   time.Time
	// justPopulated means that the entry was created, but not used by the creating caller yet.
	// Such entries are not purged, otherwise the dir we have just populated may be deleted
	// before the caller gets a chance to use it. The flag is reset by the first useEntry call.
	justPopulated bool
}
...
func (c *Cache) populateEntry(dir, metaFile, desc string, populate func(string) error) error {
	...
	c.entries[dir] = &cacheEntry{
		...
		size:     size,
		lastUsed: c.timeNow(),
		// Protect the entry from purging until the caller accounts its use.
		justPopulated: true,
	}
	c.currentSize += size
...
func (c *Cache) useEntry(dir, metaFile string) error {
	...
	if entry == nil {
		...
		// (the entry we have populated ourselves is protected by justPopulated).
	...
	// The caller accounts its use below, the entry does not need the protection anymore.
	entry.justPopulated = false
	now := c.timeNow()
...
func (c *Cache) purge() error {
	...
	for _, entry := range list {
		...
		if entry.justPopulated {
			// The entry is about to be used by the caller that has populated it.
			continue
		}
