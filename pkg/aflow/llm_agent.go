...
func (meta *LLMAgent) verify(vctx *verifyContext) {
	...
	for _, name := range promptPlaceholderRe.FindAllStringSubmatch(meta.Instruction, -1) {
		vctx.requireInput(meta.Name, name[1], reflect.TypeFor[string]())
	}
	...
	vctx.provideOutput(meta.Name, meta.OutputKey, reflect.TypeFor[string](), true)
}
...
func (a *llmAgentInstance) afterAgent(ctx agent.CallbackContext) (*genai.Content, error) {
	if false {
...
func (a *llmAgentInstance) beforeModel(ctx agent.CallbackContext, req *model.LLMRequest) (*model.LLMResponse, error) {
	...
	for _, preceding := range span.Parent.Nested {
		if preceding == span {
			break
		}
		switch {
		case preceding.Start.LLMRequest != nil:
		case preceding.Start.ToolCall != nil:
			call := preceding.Start.ToolCall
			req.Contents = append(req.Contents,
				genai.NewContentFromFunctionCall(call.Name, call.Args, genai.RoleModel),
				genai.NewContentFromFunctionResponse(call.Name, preceding.End.ToolResult.Results, genai.RoleUser),
			)
		default:
			panic(fmt.Sprintf("unexpected event type: %+v", *preceding.Start))
		}
	}
...
func (a *llmAgentInstance) afterModel(ctx agent.CallbackContext, resp *model.LLMResponse, llmError error) (
