...
type LLMAgent struct {
	...
	// InitialHistoryProvider is an optional callback function that dynamically
	// supplies the starting conversation history slice ([]llmMessage) for the
	// agent session using Context. If set, it overrides default Prompt
	// initialization and skips Prompt template verification.
	InitialHistoryProvider func(*Context) ([]llmMessage, error)
...
func (a *agentSession) chat(ctx *Context, cfg *backend.GenerateConfig, tools map[string]Tool,
	...
	if a.InitialHistoryProvider != nil {
		var err error
		a.req, err = a.InitialHistoryProvider(ctx)
		if err != nil {
			return "", nil, err
		}
	} else {
	...
	for iter := 0; iter < a.maxIterations || a.tryAnswerNow(cfg, false); iter++ {
		...
		if len(calls) == 0 {
			reply, outputs, retry, err := a.handleFinalReply(ctx, reply)
			if err != nil {
				return "", nil, err
			}
			if retry {
