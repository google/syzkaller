...
- **`IsProbe`**: A boolean indicating whether the currently generated program is a minimal capability probe (true) or a full reproducer candidate (false).
- **`ProbePassed`**: A boolean indicating if the executed minimal capability probe successfully passed all VM checks.
- **`CapabilitiesVerified`**: A persisted boolean tracking whether environment capability validation has been successfully established.
- **`TerminalError`**: A descriptive error string generated if a terminal environmental or hardware blocker is encountered (e.g., missing core devices/modules).
...
- **`ExecutionCachedID`**: A unique hash ID pointing to a persistently cached object (such as code coverage) collected during a test run.
