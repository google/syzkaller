...
func readTestCases(dir string) ([]testData, error) {
	...
	for _, dirent := range dirents {
		...
		testCase, err := readTestData(path.Join(dir, dirent.Name()))
		if err != nil {
			return nil, err
		}
...
func testGenerationImpl(t *testing.T, test testData, target *prog.Target) (string, bool) {
	p, err := target.Deserialize([]byte(test.input), prog.NonStrict)
	if err != nil {
		t.Fatal(err)
	}
	...
	for i, call := range test.calls {
		outputBuilder.WriteString(call.comment + "\n")
		outputBuilder.WriteString(call.syscall)
