...
var (
	...
	statNewInput      uint64
	statEnteredSmash  uint64

	statByteHints          uint64
	statWordHints          uint64
	statDWordHints         uint64
	statQWordHints         uint64
	statExecHints          uint64
	statExecHintSeeds      uint64
	statHintsWithNewSignal uint64
	statHintsNewSignals    uint64

	allTriaged            uint32
	noCover               bool
	faultInjectionEnabled bool
	kcovCompsEnabled      bool
)
...
func main() {
	...
	if fd, err := syscall.Open("/sys/kernel/debug/kcov", syscall.O_RDWR, 0); err == nil {
		kcovCompsEnabled = func(fd int) bool {
			coverSize := uintptr(64 << 10)
			_, _, errno := syscall.Syscall(syscall.SYS_IOCTL, uintptr(fd), sys.KCOV_INIT_TRACE, coverSize)
			if errno != 0 {
				Logf(1, "KCOV_CHECK: KCOV_INIT_TRACE = %v\n", errno)
				return false
			}
			_, err := syscall.Mmap(fd, 0, int(coverSize*8), syscall.PROT_READ|syscall.PROT_WRITE, syscall.MAP_SHARED)
			if err != nil {
				Logf(1, "KCOV_CHECK: mmap = %v\n", err)
				return false
			}
			KCOV_MODE_TRACE_CMP := uintptr(2)
			_, _, errno = syscall.Syscall(syscall.SYS_IOCTL, uintptr(fd), sys.KCOV_ENABLE, KCOV_MODE_TRACE_CMP)
			Logf(1, "KCOV_CHECK: KCOV_ENABLE = %v\n", errno)
			return (errno == 0)
		}(fd)
		Logf(1, "KCOV_CHECK: kcovCompsEnabled=%v\n", kcovCompsEnabled)
		syscall.Close(fd)
	}
	if r.NeedCheck {
		a := &CheckArgs{
	...
	if kcovCompsEnabled {
		config.Flags |= ipc.FlagEnableComps
	}
			...
			for i := 0; ; i++ {
				...
				if len(triageCandidate) != 0 || len(candidates) != 0 || len(triage) != 0 || len(smashQueue) != 0 {
					...
					if len(triageCandidate) != 0 {
						...
						execute(pid, env, candidate.p, false, false, candidate.minimized, true, false, &statExecCandidate)
						continue
				...
				if len(corpus) == 0 || i%100 == 0 {
					...
					execute(pid, env, p, false, false, false, false, false, &statExecGen)
				} else if kcovCompsEnabled && len(corpus) != 0 && i%10 == 0 {
					corpusMu.RUnlock()
					Logf(1, "Hints mutation strategy chosen\n")
					p := corpus[rnd.Intn(len(corpus))]
					executeHintSeed(pid, env, p)
				} else {
					...
					execute(pid, env, p, false, false, false, false, false, &statExecFuzz)
				}
			}
		}()
	}
	...
	for {
		...
		if poll || time.Since(lastPoll) > 10*time.Second {
			...
			execHints := atomic.SwapUint64(&statExecHints, 0)
			execTotal += execHints
			a.Stats["exec hints"] = execHints
			execHintSeeds := atomic.SwapUint64(&statExecHintSeeds, 0)
			execTotal += execHintSeeds
			a.Stats["exec hints seeds"] = execHintSeeds
			hintsWithNewSignal := atomic.SwapUint64(&statHintsWithNewSignal, 0)
			a.Stats["hints that generated new signals"] = hintsWithNewSignal
			hintsSignalCount := atomic.SwapUint64(&statHintsNewSignals, 0)
			a.Stats["count of new signals generated by hints"] = hintsSignalCount
			execEnteredSmash := atomic.SwapUint64(&statEnteredSmash, 0)
			a.Stats["entered smashing"] = execEnteredSmash

			a.Stats["exec byte hints"] = atomic.SwapUint64(&statByteHints, 0)
			a.Stats["exec word hints"] = atomic.SwapUint64(&statWordHints, 0)
			a.Stats["exec dword hints"] = atomic.SwapUint64(&statDWordHints, 0)
			a.Stats["exec qword hints"] = atomic.SwapUint64(&statQWordHints, 0)
			r := &PollRes{}
...
func smashInput(pid int, env *ipc.Env, ct *prog.ChoiceTable, rs rand.Source, inp Input) {
	...
	for i := 0; i < 100; i++ {
		...
		execute(pid, env, p, false, false, false, false, false, &statExecSmash)
	}
	atomic.AddUint64(&statEnteredSmash, 1)
...
func triageInput(pid int, env *ipc.Env, inp Input) {
	...
	if inp.minimized {
		...
		inp.p, inp.call = prog.Minimize(inp.p, inp.call, func(p1 *prog.Prog, call1 int) bool {
			info := execute(pid, env, p1, false, false, false, false, false, &statExecMinimize)
...
func executeHintSeed(pid int, env *ipc.Env, p *prog.Prog) {
	...
	// and to measure initial signals
	info := execute(pid, env, p, false, true, false, false, false, &statExecHintSeeds)
	// parse the comparisons and generate the hints
	hints := p.GenerateHints(info)
	// mutate the program according to each of the hints
	// and triage it to check if it changes coverage
	for _, hint := range hints {
		newP := p.MutateWithHint(hint)
		if newP != nil {
			switch hint.Size {
			case 1:
				atomic.AddUint64(&statByteHints, 1)
			case 2:
				atomic.AddUint64(&statWordHints, 1)
			case 4:
				atomic.AddUint64(&statDWordHints, 1)
			case 8:
				atomic.AddUint64(&statQWordHints, 1)
			}
			execute(pid, env, newP, false, false, false, false, true, &statExecHints)
		}
	}
}
func execute(pid int, env *ipc.Env, p *prog.Prog, needCover, needComps, minimized, candidate, generatedByHint bool, stat *uint64) []prog.CallInfo {
	opts := &ipc.ExecOpts{}
	if needComps && kcovCompsEnabled {
		// needComps has "higher priority" then needCover, so
	...
	for i, inf := range info {
		...
		if generatedByHint {
			atomic.AddUint64(&statHintsWithNewSignal, 1)
			atomic.AddUint64(&statHintsNewSignals, uint64(len(diff)))
		}
