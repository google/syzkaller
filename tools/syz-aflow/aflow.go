...
func main() {
	...
	if err := run(context.Background(), *flagLargeModel, *flagFlow, *flagInput, *flagWorkdir); err != nil {
		tool.Fail(err)
	}
}

func run(ctx context.Context, largeModel bool, flowName, inputFile, workdir string) error {
	...
	_, err = flow.Execute(ctx, largeModel, workdir, inputs, onEvent)
	return err
}
