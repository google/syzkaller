...
if [ ! -w /dev/kvm ]; then
    echo "ERROR: /dev/kvm not found or not writable. KVM is required for nested virtualization."
    exit 1
fi
...
# Start with docker driver, larger disk, and mount parent projects directory
minikube start -p "${PROFILE}" --driver=docker --disk-size=80g --memory=16384 --cpus=8 --mount --mount-string="$(dirname "${REPO_ROOT}"):/projects"
