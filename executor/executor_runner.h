...
private:
	friend std::ostream& operator<<(std::ostream& ss, const ProcOpts& opts)
	{
		ss << "use_cover_edges=" << opts.use_cover_edges
		   ...
		   << " proc_max_execs=" << opts.proc_max_execs;
		return ss;
	}
};
...
public:
	...
	bool Execute(rpc::ExecRequestRawT& msg)
	{
		...
		if (state_ == State::Idle && ((corpus_triaged_ && restarting_ == 0 && freshness_ >= opts_.proc_max_execs) ||
					      req_type_ != msg.type ||
...
private:
	...
	int Handshake()
	{
		...
		debug("connected to manager: procs=%d cover_edges=%d kernel_64_bit=%d slowdown=%d syscall_timeout=%u"
		      " program_timeout=%u proc_max_execs=%d features=0x%llx\n",
		      conn_reply.procs, conn_reply.cover_edges, conn_reply.kernel_64_bit,
		      conn_reply.slowdown, conn_reply.syscall_timeout_ms,
		      conn_reply.program_timeout_ms, conn_reply.proc_max_execs, static_cast<uint64>(conn_reply.features));
		...
		proc_opts_.proc_max_execs = conn_reply.proc_max_execs;
		if (proc_opts_.proc_max_execs == 0)
			fail("invalid proc_max_execs value");
		if (conn_reply.cover)
			max_signal_.emplace();
