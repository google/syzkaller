...
guest_handle_nested_vmentry_intel(uint64 vm_id, uint64 cpu_id, bool is_launch)
{
	...
	2:	// 7. Failure path.
		...
		pop %%rbp
		pop %%rbx
		add $16, %%rsp // pop launch and cpu_id
		add $128, %%rsp // restore Red Zone
		mov $1, %[ret]
		jmp 3f
...
guest_run_amd_vm(uint64 cpu_id, uint64 vm_id)
{
	...
	asm volatile(R"(
		...
		// 2. Stack Passthrough for Exit Handler.
		push %[cpu_id]
		push %[vmcb_addr] // Save VMCB address for later use after VMEXIT.
	...
	1:	// Host resumes here.
		...
		// 6. Save L2's GPRs.
		pushq 0x70(%%rax) // exit_code (it will be at Index 15)
