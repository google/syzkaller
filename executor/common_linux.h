...
// Given a sndctrl fp and a random integer, return a valid
// (struct snd_ctl_elem_id).numid value that belongs to an existing kctl.
static long syz_sndrv_get_elem_id(volatile long a0, volatile long a1)
{
	// syzlang: syz_sndrv_get_elem_id(fd fd_sndctrl, elem_id_rand int32)	numid
	// C:       syz_sndrv_get_elem_id(int fd_sndctrl, unsigned int elem_id_rand)
	...
	// Randomly pick an elem id: elem indices are in the [1, elems_count] range
