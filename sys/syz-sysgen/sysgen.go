	...
	for _, OS := range OSList {
		...
		for _, job := range jobs {
			...
			go func() {
				...
				job.ArchData = generateExecutorSyscalls(job.Target, prog.Decls, prog.Syscalls, rev)
...
func generateExecutorSyscalls(target *targets.Target, decls []*prog.CDecl, syscalls []*prog.Syscall, rev string) ArchData {
	data := ArchData{
		Revision:   rev,
		GOARCH:     target.Arch,
...
{{range $arch := $os.Archs}}
#if GOARCH_{{$arch.GOARCH}}
{{range $decl := $arch.Decls}}{{$decl.Body}}
{{end}}
