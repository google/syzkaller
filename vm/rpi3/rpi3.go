...
func init() {
	vmimpl.Register("rpi3", ctor, false)
}

type Config struct {
	Host_Addr  string // ip address of the host machine
	Slave_Addr string // ip address of the RPI3 board
...
func ctor(env *vmimpl.Env) (vmimpl.Pool, error) {
	...
	if err := config.LoadData(env.Config, cfg); err != nil {
		return nil, fmt.Errorf("failed to parse rpi3 vm config: %v", err)
	}
	if cfg.Host_Addr == "" {
...
func (pool *Pool) Create(workdir string, index int) (vmimpl.Instance, error) {
	...
	//remount read-only file system to writable
	inst.ssh("mount -o remount,rw /")
	inst.ssh("mount -o remount,rw /var")
	inst.ssh("mount -o remount,rw /sys")
...
func (inst *instance) waitForSSH(timeout time.Duration) error {
	return vmimpl.WaitForSSH(inst.debug, timeout, inst.cfg.Slave_Addr, inst.sshkey, "root", inst.os, 22, nil)
}
