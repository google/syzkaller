...
// TestExtractMultipleErrors is a smoke integration test.
// If you want to test chained reports parsing, use pkg/report/testdata/parse_all instead.
func TestExtractMultipleErrors(t *testing.T) {
	...
	mon := &monitor{
		...
		inst:       inst,
		reporter:   reporter,
		output:     []byte(multiReport),
	}
	reps := mon.extractErrors("unknown error")
	assert.Len(t, reps, 2, "expected to see 2 reports, got %v", len(reps))
	assert.Equal(t, "[ T5752]", reps[0].ContextID,
		"expected ContextID [ T5752], got %v", reps[0].ContextID)
	assert.Equal(t, reps[0].ContextID, reps[1].ContextID, "ContextID differ")
}

//go:embed test_data/multi_report.txt
